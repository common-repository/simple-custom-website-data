    ...
    public function add()
    {
        if (isset($_POST['ref']) && !empty($_POST['ref']) && isset($_POST['data']) && !empty($_POST['data']) && $_POST['cwdaction'] == 'add')
        {
            if ( !empty($_POST) && check_admin_referer('cwd_add_action','cwd_add_name') )
            {
                ...
                $existingRecord = $this->database->getByRef($ref);
                if(!is_null($existingRecord)) {
                    $format = 'The record \'%s\' already exists. <a href="%s&view=edit&id=%s">Want to edit it?</a>';
                    $format .= '<style>div.updated{border-color: #ffba00}</style>'; // dirty hack until a rewrite
                    $message = sprintf($format, $existingRecord->ref, CWD_URL, $existingRecord->id);
